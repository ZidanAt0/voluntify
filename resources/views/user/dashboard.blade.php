        ...
        <div class="p-6 sm:p-8 flex items-center justify-between">
            ...
            <div class="hidden sm:flex items-center gap-6">
                <a href="{{ route('events.index') }}"
                    class="px-4 py-2 rounded-xl bg-white text-indigo-700 hover:bg-gray-50">Jelajahi Event</a>
            </div>
        </div>
    </div>
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm ring-1 ring-gray-200 p-6">
                <div class="flex items-center justify-between">
                    ...
                    <a href="{{ route('registrations.index') }}" class="text-sm text-indigo-700 hover:underline">Lihat
                        semua</a>
                </div>

                @if ($upcomingRegs->isEmpty())
                    <p class="text-gray-600 mt-3">Belum ada event mendatang. Coba jelajahi <a
                            class="text-indigo-700 hover:underline" href="{{ route('events.index') }}">daftar event</a>.</p>
                @else
                        ...
                        @foreach ($upcomingRegs as $r)
                            ...
                            <div class="py-3 flex items-start gap-4">
                                ...
                                <div class="flex-1">
                                    <div class="flex items-center gap-2">
                                        <a href="{{ route('events.show', $e->slug) }}"
                                            class="font-medium hover:underline">{{ $e->title }}</a>
                                        @if ($e->category)
                                            <span
                                                class="text-xs px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-700 ring-1 ring-indigo-200">{{ $e->category->name }}</span>
                                        @endif
                                    </div>
                                ...
                                <a href="{{ route('registrations.show', $r) }}"
                                    class="text-sm px-3 py-1.5 rounded-lg border border-indigo-200 text-indigo-700 hover:bg-indigo-50">Detail</a>
                            </div>
                        @endforeach
                    </div>
                @endif
                        ...
                        @foreach ($recentRegs as $r)
                            ...
                            <li class="p-3 rounded-xl ring-1 ring-gray-200 flex items-center justify-between">
                                <div class="min-w-0">
                                    ...
                                    <div class="text-sm text-gray-600">{{ $e->date_human }} · {{ $e->location_human }}
                                    </div>
                                </div>
                                <span
                                    class="ml-4 inline-flex px-2.5 py-0.5 rounded-full text-xs ring-1 {{ $badge }}">
        ...
        <aside class="space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm ring-1 ring-gray-200 p-6">
                <div class="flex items-center justify-between">
                    ...
                    <a href="{{ route('bookmarks.index') }}" class="text-sm text-indigo-700 hover:underline">Lihat
                        semua</a>
                </div>
                ...
                @else
                    <div class="mt-4 grid grid-cols-1 gap-3">
                        @foreach ($bookmarkedEvents as $e)
                            <a href="{{ route('events.show', $e->slug) }}"
                                class="flex items-center gap-3 p-2 rounded-xl ring-1 ring-gray-200 hover:bg-gray-50">
                                <img src="{{ $e->banner_url }}"
                                    class="w-16 h-12 object-cover rounded-md ring-1 ring-gray-200" alt="">
                                <div class="min-w-0">
                                    <div class="text-sm font-medium truncate">{{ $e->title }}</div>
                                    <div class="text-xs text-gray-600 truncate">{{ $e->date_human }} ·
                                        {{ $e->location_human }}</div>
                                </div>
                            </a>
                        @endforeach
                    </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm ring-1 ring-gray-200 p-6">
                ...
                <div class="mt-3 grid gap-2">
                    <a href="{{ route('events.index') }}"
                        class="px-4 py-2 rounded-xl border border-gray-200 hover:bg-gray-50">Cari Event</a>
                    <a href="{{ route('registrations.index') }}"
                        class="px-4 py-2 rounded-xl border border-gray-200 hover:bg-gray-50">Pendaftaran Saya</a>
                    <a href="{{ route('bookmarks.index') }}"
                        class="px-4 py-2 rounded-xl border border-gray-200 hover:bg-gray-50">Bookmark</a>
