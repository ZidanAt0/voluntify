    ...
    <form method="GET" class="mt-6 bg-white rounded-2xl shadow-sm ring-1 ring-gray-200 p-4 grid sm:grid-cols-5 gap-3">
        ...
        <div class="sm:col-span-1 flex gap-2">
            ...
            <a href="{{ route('registrations.index') }}"
                class="px-4 py-2 rounded-xl border border-gray-200 hover:bg-gray-50">Reset</a>
        </div>
    </form>
    ...
    <div class="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        @forelse ($regs as $r)
            ...
            <div class="bg-white rounded-2xl shadow-md ring-1 ring-gray-200 overflow-hidden flex flex-col">
                <a href="{{ route('events.show', $e->slug) }}"><img src="{{ $e->banner_url }}"
                        class="w-full h-40 object-cover" loading="lazy"></a>
                <div class="p-4 flex-1">
                    ...
                    @if ($e->category)
                        <div class="mt-1">
                            <span
                                class="text-xs px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-700 ring-1 ring-indigo-200">
                                {{ $e->category->name }}
                            </span>
                        </div>
                    @endif
                    ...
                    <ul class="mt-3 space-y-1.5 text-sm text-gray-700">
                        ...
                        <li class="flex items-center gap-2">
                            <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none">
                                <path d="M12 21s7-4.35 7-10a7 7 0 1 0-14 0c0 5.65 7 10 7 10z" stroke="currentColor"
                                    stroke-width="1.5" />
                ...
                <div class="px-4 pb-4 flex items-center gap-2">
                    ...
                    @if ($r->cancellable())
                        <form method="POST" action="{{ route('registrations.cancel', $r) }}"
                    ...
                    @else
                        <button disabled class="px-3 py-2 rounded-xl border border-gray-200 text-gray-500">Tidak bisa
                            batalkan</button>
                    @endif
                </div>
            </div>
        @empty
            <div class="sm:col-span-2 lg:col-span-3">
                <div class="rounded-2xl bg-white ring-1 ring-gray-200 p-8 text-center text-gray-600">
                    Belum ada pendaftaran. <a class="text-indigo-700 hover:underline"
                        href="{{ route('events.index') }}">Jelajahi event</a>.
                </div>
            </div>
        @endforelse
    </div>
