        ...
        <form method="POST" action="{{ route('registrations.apply', $event) }}" class="p-6 grid gap-6">
            ...
            <div class="grid sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input value="{{ $user->name }}" disabled class="mt-1 w-full rounded-xl border-gray-200 bg-gray-50">
                    <p class="text-xs text-gray-500 mt-1">Ubah di halaman Profil jika perlu.</p>
                </div>
                <div>
                    <label class="block text-sm text-gray-700">Email</label>
                    <input value="{{ $user->email }}" disabled class="mt-1 w-full rounded-xl border-gray-200 bg-gray-50">
                </div>
            </div>
            ...
            <div>
                ...
                <input name="whatsapp" value="{{ old('whatsapp', $user->whatsapp) }}" required
            ...
            <div>
                ...
                <input name="city" value="{{ old('city', $user->city) }}" class="mt-1 w-full rounded-xl border-gray-300">
            ...
            {{-- Pertanyaan inti --}}
            <div>
                <label class="block text-sm text-gray-700">Motivasi mengikuti event ini <span class="text-red-600">*</span></label>
                <textarea name="motivation" rows="4" class="mt-1 w-full rounded-xl border-gray-300" required>{{ old('motivation') }}</textarea>
                @error('motivation')<p class="text-sm text-red-600">{{ $message }}</p>@enderror
            </div>
            ...
            <div>
                ...
                @error('experience')<p class="text-sm text-red-600">{{ $message }}</p>@enderror
            </div>

            <div>
                <label class="block text-sm text-gray-700">Keahlian (pilih yang sesuai, opsional)</label>
                @php
                    $skillOptions = ['Logistik','Public Speaking','Desain Grafis','Fotografi/Videografi','Administrasi','Pertolongan Pertama'];
                ...
                <div class="mt-2 grid sm:grid-cols-2 gap-2">
                    ...
                    <label class="inline-flex items-center gap-2 rounded-xl border border-gray-200 px-3 py-2">
                        <input type="checkbox" name="skills[]" value="{{ $skill }}" @checked(in_array($skill, $oldSkills))>
                        <span>{{ $skill }}</span>
                    </label>
                    @endforeach
                </div>
                @error('skills')<p class="text-sm text-red-600">{{ $message }}</p>@enderror
            </div>

            <div class="grid sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm text-gray-700">Kontak Darurat <span class="text-red-600">*</span></label>
                    <input name="emergency_name" placeholder="Nama" value="{{ old('emergency_name') }}" required
                        class="mt-1 w-full rounded-xl border-gray-300">
                    @error('emergency_name')<p class="text-sm text-red-600">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="block text-sm text-gray-700">No. Kontak Darurat <span class="text-red-600">*</span></label>
                    <input name="emergency_phone" placeholder="08xxxxxxxxxx" value="{{ old('emergency_phone') }}" required
                        inputmode="numeric" pattern="\d{9,15}"
                        class="mt-1 w-full rounded-xl border-gray-300">
                    <p class="text-xs text-gray-500 mt-1">9–15 digit angka. Tidak boleh sama dengan WhatsApp.</p>
                    @error('emergency_phone')<p class="text-sm text-red-600">{{ $message }}</p>@enderror
                </div>
            </div>

            <label class="flex items-start gap-2">
                <input type="checkbox" name="agree" value="1" class="mt-1" {{ old('agree') ? 'checked' : '' }} required>
