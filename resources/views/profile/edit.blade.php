        ...
        <div class="p-6 sm:p-8 flex items-center gap-5">
            <div class="relative">
                <img id="avatarPreview" src="{{ auth()->user()->avatar_url }}"
                    class="w-20 h-20 sm:w-24 sm:h-24 rounded-full object-cover ring-4 ring-white/20" alt="avatar">
                <!-- tombol pensil -->
    ...
    <div class="mt-6 bg-white rounded-2xl shadow-sm ring-1 ring-gray-200 p-6">
        <form id="profile-form" method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data"
            ...
            <div>
                ...
                <input name="whatsapp" value="{{ old('whatsapp', auth()->user()->whatsapp) }}"
                    class="mt-1 w-full rounded-xl border-gray-300">
                @error('whatsapp')
                    <p class="text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div class="sm:col-span-2">
                ...
                <input name="interests" value="{{ old('interests', implode(', ', auth()->user()->interests ?? [])) }}"
                    class="mt-1 w-full rounded-xl border-gray-300">
                @error('interests')
                    <p class="text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div class="sm:col-span-2">
                <button class="px-5 py-2.5 rounded-xl bg-indigo-600 text-white hover:bg-indigo-700">Simpan
                    Perubahan</button>
            </div>
        </form>
    </div>
